...
including the SDE/ODE integrators and transport path definitions.
"""
...
from src.utils import timeshift
...
Choice_SamplingODE = Literal["euler"]
...
@dataclass
class ODEConfig:
    sampling_method: Choice_SamplingODE = "euler"
    atol: float = 1e-6
    rtol: float = 1e-3
    reverse: bool = False
...
def sampling_grid(num_steps, shift=1.0):
    """`num_steps` Euler intervals spanning the whole path, noise (t=0) to data.

    Self-Flow's sampleshift warps the uniform grid with the same timeshift used
    for training timesteps, concentrating steps at high noise. The shift fixes
    both endpoints, so the grid still starts at pure noise and lands exactly on
    clean data -- unlike the SDE path, no separate final step is needed.
    """
    return timeshift(torch.linspace(0, 1, num_steps + 1), shift)


class ode:
    """Euler ODE solver over an explicit, possibly non-uniform timestep grid."""

    def __init__(self, drift, *, t):
        self.t = t
        self.drift = drift
    ...
    def sample(self, init, model, **model_kwargs):
        x = init
        samples = [x]
        for t_cur, t_next in zip(self.t[:-1], self.t[1:]):
            t = torch.ones(x.size(0)).to(x) * t_cur
            with torch.no_grad():
                x = x + self.drift(x, t, model, **model_kwargs) * (t_next - t_cur)
            samples.append(x)
        return samples
...
def denoise_loop(
    ...
    **model_kwargs,
    ...
    if mode == "SDE":
        # The SDE grid ends at 1 - last_step_size and closes the gap with a
        # fixed-size final step, which a shifted grid would no longer line up
        # with; sampleshift is an ODE-mode option.
        assert shift == 1.0, "sampleshift is only implemented for ODE mode"
        ...
        # Velocity prediction, so the drift is the model output itself.
        sample_fn = ode(
            lambda x, t, model, **kwargs: model(x, t, **kwargs),
            t=sampling_grid(num_steps, shift),
        ).sample
    else:
        raise NotImplementedError(f"Unknown sampling mode {mode!r}")
