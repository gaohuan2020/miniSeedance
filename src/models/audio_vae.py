...
#
# Stable Audio Open 1.0 VAE (Oobleck encoder/decoder + VAE bottleneck),
# ported from daVinci-MagiHuman inference/model/sa_audio/ with the
# distributed/offload machinery stripped. The StableAudioVAE wrapper at the
# bottom loads VAE weights (prefix `pretransform.model.`) from the full
# stable-audio-open checkpoint (model_config.json + model.safetensors).
#
# Stable Audio Open 1.0: 44.1 kHz stereo -> latent (B, 64, T/2048).
...
class StableAudioVAE:
    """Audio encoder/decoder wrapper (adapted from MagiHuman's SAAudioFeatureExtractor).

    Loads only the VAE from a local stable-audio-open-1.0 checkpoint directory
    (model_config.json + model.safetensors), skipping the T5 and diffusion parts.
    """

    def __init__(self, model_path="checkpoints/stable-audio-open-1.0", device="cuda", dtype=torch.float32):
        ...
        self.sample_rate = full_config["sample_rate"]
        autoencoder_config = {
            "model_type": "autoencoder",
            "sample_rate": self.sample_rate,
            "model": full_config["model"]["pretransform"]["config"],
        }
        model = create_model_from_config(autoencoder_config)

        # VAE weights live under the `pretransform.model.` prefix of the full checkpoint
        prefix = "pretransform.model."
        full_state_dict = load_file(model_path / "model.safetensors")
        vae_state_dict = {k[len(prefix):]: v for k, v in full_state_dict.items() if k.startswith(prefix)}
        model.load_state_dict(vae_state_dict)

        self.device = device
        self.dtype = dtype
        self.model = model.to(device=device, dtype=dtype).eval().requires_grad_(False)
        self.latent_dim = self.model.latent_dim              # 64
        self.downsampling_ratio = self.model.downsampling_ratio  # 2048
        self.io_channels = self.model.io_channels            # 2 (stereo)

    @torch.no_grad()
    def encode(self, waveform):
        """(B, 2, N) in [-1, 1] at 44.1 kHz -> latents (B, 64, N/2048)."""
        return self.model.encode(waveform.to(self.device, self.dtype))

    @torch.no_grad()
    def decode(self, latents):
        """Latents (B, 64, L) -> waveform (B, 2, L*2048) in [-1, 1]."""
        return self.model.decode(latents.to(self.device, self.dtype))
