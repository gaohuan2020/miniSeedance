"""
Wan2.2 3D causal video VAE, copied from daVinci-MagiHuman
(inference/model/vae2_2/vae2_2_module.py), which is itself the Wan2.2 VAE.

Changes vs the reference: the distributed (sequence-parallel) code paths and
@torch.compile decorators are stripped; the architecture, causal temporal
cache and chunked encode/decode logic are unchanged, so the official
Wan2.2_VAE.pth checkpoint loads directly.

Compression: 16x spatial (pixel patchify 2 + 8x conv), 4x temporal,
z_dim=48. Input video: (B, 3, T, H, W) in [-1, 1] with T = 1 + 4k.
"""
...
# Per-channel latent statistics of the Wan2.2 VAE (from daVinci-MagiHuman)
WAN22_LATENT_MEAN = [
...
class Wan2_2_VAE:
    """Frozen Wan2.2 video VAE with normalized latents (from daVinci-MagiHuman)."""
    ...
    def __init__(
        ...
        self.mean = torch.tensor(WAN22_LATENT_MEAN, dtype=dtype, device=device)
        self.std = torch.tensor(WAN22_LATENT_STD, dtype=dtype, device=device)
        ...
        model.load_state_dict(torch.load(vae_pth, map_location="cpu", weights_only=True))
        self.vae = model.eval().requires_grad_(False).to(device=device, dtype=dtype)

    @torch.no_grad()
    def encode(self, video):
        """(B, 3, T, H, W) in [-1, 1], T = 1 + 4k -> (B, 48, 1 + k, H/16, W/16)."""
        return self.vae.encode(video.to(self.device, self.dtype), self.scale).float()

    @torch.no_grad()
    def decode(self, z):
        """Normalized latents -> video (B, 3, T, H, W) in [-1, 1]."""
        return self.vae.decode(z.to(self.device, self.dtype), self.scale).float().clamp_(-1, 1)
